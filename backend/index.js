...
app.post("/users", jwtAuth, async (req, res) => {
    try{
        ...
        if (Object.keys(rest).length > 0) {
            return res.status(400).json({ error: `Unexpected fields: ${Object.keys(rest).join(", ")}` });
        }
        ...
        if (utorid.length !== 8) {
            return res.status(400).json({ error: "utorid must be 8 characters long" });
        }

        if (!/^[a-zA-Z0-9]+$/.test(utorid)) {
            return res.status(400).json({ error: "utorid must only contain alphanumeric characters" });
        }
        ...
        user = await prisma.user.create({
            data: { utorid, name, email, password: uuidv4(), role: "regular", expiresAt, resetToken: uuidv4() }
        });
        let response = {"id": user.id,
        "utorid": user.utorid,
        "name": user.name,
        "email": user.email,
...
app.get("/users", jwtAuth, async (req, res) => {
    try {
        ...
        if (verified !== undefined) {
            ...
            return res.status(400).json({ error: "Query parameter 'verified' must be a boolean." });
            }
        ...
        if (activated !== undefined) {
            ...
            return res.status(400).json({ error: "Query parameter 'activated' must be a boolean." });
            }
    ...
    try {
        ...
        if (birthday) {
            ...
            if (
              ...
              return res.status(400).json({ error: "Invalid birthday. Please check the month and day." });
            }
        }
...
app.patch("/users/me/password", jwtAuth, async (req, res) => {
    try {
        ...
        if (old === undefined || old === null || newPassword === null || newPassword === undefined) {
            return res.status(400).json({ error: "missing fields"});
        }
        ...
        if (!regex.test(newPassword)) {
            return res.status(400).json({ error: "New password must be 8-20 characters, at least one uppercase, one lowercase, one number, and one special character" });
        }
...
app.patch("/users/:userId", jwtAuth, async (req, res) => {
    try{
        ...
        if (role !== undefined && role !== null) {
            ...
            if (req.user.role === "manager" && !["cashier", "regular"].includes(role)) {
                return res.status(403).json({ error: "role should either cashier or regular."});
            }
            if (!["regular", "cashier", "manager", "superuser"].includes(role)) {
                return res.status(400).json({ error: "role should any of cashier, regular, manager, or superuser"});
            }
            data.role = role;
        }
        ...
        res.json({id: updatedUser.id, utorid: updatedUser.utorid, name: updatedUser.name, ...data});
    }
    catch (error) {
        res.status(500).json({ error });
    }
...
app.post("/auth/resets", async (req, res) => {
    try {
        ...
        const expiresAt = new Date();
        expiresAt.setHours(expiresAt.getHours() + 1);
        await prisma.user.update({where: {id: user.id}, data: {resetToken, expiresAt}});
...
app.post("/auth/resets/:resetToken", async (req, res) => {
    try {
        ...
        if (resetToken !== user.resetToken) {
            const existingToken = await prisma.user.findFirst({where: {resetToken}});
            if (existingToken) {
    ...
    try{
        ...
        if (type === "purchase") {
            ...
            if (typeof spent !== "number" || isNaN(spent) || spent <= 0) {
                return res.status(400).json({ error: "spent must be a positive numeric value"});
            }
            ...
            if (promotionIds !== undefined && promotionIds !== null) {
                ...
                if (promotionIds.length > 0) {
                    ...
                    if (validPromotions.length !== promotionIds.length) {
                        return res.status(400).json({ error: "One or more promotions are invalid, expired" });
                    }
                    if (validPromotions.some(p => p.minSpending > spent)) {
                        return res.status(400).json({ error: "promotion MIN_SPEND_NOT_MET"});
                    }
                    data.promotionIds = promotionIds;
                    userData.used = user.used.concat(validPromotions.filter(p => p.type === "one-time").map(p => p.id));
                    ...
                    automaticPromotions = automaticPromotions.filter(p => !promotionIds.includes(p.id));
        ...
        const { utorid, amount, relatedId, promotionIds, remark, ...rest } = req.body;
        if (Object.keys(rest).length > 1) {
            return res.status(400).json({ error: `Unexpected fields: ${Object.keys(rest).join(", ")}` });
        }
        if (utorid === undefined || utorid === null) {
        ...
        const related = await prisma.transaction.findUnique({ where: {id: relatedId } });
...
app.get("/transactions", jwtAuth, async (req, res) => {
    try{
        ...
        const suspicious =
        req.query.suspicious === "true" ? true :
        req.query.suspicious === "false" ? false :
        undefined;
...
app.get("/transactions/:transactionId", jwtAuth, async (req, res) => {
    try{
        ...
        const result = Object.fromEntries(Object.entries(transaction).filter(([_, value]) => value !== null));
        res.json(result);
    }
    catch(error) {
        console.error("transaction get error:", error);
...
app.patch("/transactions/:transactionId/suspicious", jwtAuth, async (req, res) => {
    try {
        ...
        const updated = await prisma.transaction.update({ where: { id }, data: { suspicious },
            select: {
                id: true,
                utorid: true,
                amount: true,
        ...
        const result = Object.fromEntries(Object.entries(updated).filter(([_, value]) => value !== null));
        await prisma.user.update({ where: { utorid: transaction.utorid }, data: { points: { increment: amount } } });
        res.json(result);
    }
    catch(error) {
...
app.patch("/transactions/:transactionId/processed", jwtAuth, async (req, res) => {
    try {
        ...
        if (transaction.type !== "redemption" || transaction.processedBy !== null) {
            return res.status(400).json({ error: "Bad request" });
        }
        const amount = transaction.amount;
        
        const updated = await prisma.transaction.update({ where: { id }, data: { processedBy: req.user.utorid },
            select: {
                id: true,
                utorid: true,
                type: true,
...
app.post("/events", jwtAuth, async (req, res) => {
    try {
        ...
        const { name, description, location, startTime, endTime, capacity, points, ...rest } = req.body;
        if (Object.keys(rest).length > 0) {
            return res.status(400).json({ error: `Unexpected fields: ${Object.keys(rest).join(", ")}` });
        }
        ...
        if (typeof startTime !== "string" || !startTime) {
            return res.status(400).json({ error: "startTime must be a non-empty string" });
        }
        if (typeof endTime !== "string" || !endTime) {
            return res.status(400).json({ error: "endTime must be a non-empty string" });
        }
        ...
        const data = { name, description, location, startTime: start, endTime: end, pointsRemain: points, pointsAwarded: 0 };
...
app.get("/events", jwtAuth, async (req, res) => {
    try {
        ...
        const events = await prisma.event.findMany({ where, skip, take: limit, include: {guests: true} });
        ...
        if (!clearance.includes(req.user.role)) {
            ...
            const results = events.map(e => {
                return {id: e.id, name: e.name, location: e.location, 
                    startTime: e.startTime, endTime: e.endTime, 
                    capacity: e.capacity, pointsRemain: e.pointsRemain,
                    pointsAwarded: e.pointsAwarded, published: e.published, 
...
app.get("/events/:eventId", jwtAuth, async (req, res) => {
    try{
        ...
        if (clearance.includes(req.user.role) || event.organizers.filter(o => o.id === req.user.id).length > 0) {
            return res.json({id: event.id, name: event.name, 
                description: event.description, location: event.location, 
                startTime: event.startTime, endTime: event.endTime, 
                capacity: event.capacity, pointsRemain: event.pointsRemain,
                pointsAwarded: event.pointsAwarded, published: event.published, 
                organizers, guests
            });
        }
        else {
...
app.patch("/events/:eventId", jwtAuth, async (req, res) => {
    try {
        ...
        const { name, description, location, startTime, endTime, capacity, points, published, ...rest } = req.body;
        if (Object.keys(rest).length > 0) {
            return res.status(400).json({ error: `Unexpected fields: ${Object.keys(rest).join(", ")}` });
        }
        const data = {};
        ...
        if (capacity !== undefined && capacity !== null) {
            ...
            if (capacity < numGuests) {
                return res.status(400).json({ error: "capacity cannot be less than guests"});
            }
            data.capacity = capacity;
            if (capacity === numGuests) {
                data.full = true;
        ...
        if ((name || description || location || startTime || capacity) && now >= oldStart) {
...
app.delete("/events/:eventId", jwtAuth, async (req, res) => {
    try {
        ...
        if (event.published) {
            return res.status(400).json({ error: "Event has already been published" });
        }
...
app.post("/events/:eventId/organizers", jwtAuth, async (req, res) => {
    try {
        ...
        if (event.guests.some( guest => guest.id === user.id)) {
            return res.status(400).json({ error: "user is registered as a guest to the event (remove user as guest first, then retry)" });
        }
        const now = new Date();
        ...
        return res.status(201).json({id: updated.id, name: updated.name, location: updated.location, organizers});
    }
    catch(error) {
...
app.post("/events/:eventId/guests/me", jwtAuth, async (req, res) => {
    try {
        ...
        const guestAdded = { id: req.user.id, utorid: req.user.utorid, name: req.user.name, };
        const numGuests = updated.guests.length;  
        return res.status(201).json({id: updated.id, name: updated.name, location: updated.location, 
            guestAdded, numGuests
        });
    }
    catch(error) {
        console.error("eventid organizer error:", error);
...
app.post("/events/:eventId/guests", jwtAuth, async (req, res) => {
    try {
        ...
        if (!clearance.includes(req.user.role) && !event.organizers.some(o => o.id === req.user.id)) {
            return res.status(403).json({ error: "Forbidden" });
        }
...
app.post("/events/:eventId/transactions", jwtAuth, async (req, res) => {
    try{
        ...
        if (user) {
            ...
            return res.status(201).json({
                id: transaction.id, recipient: user.utorid, awarded: amount,
                type, relatedId: event.id, remark: event.description,
                createdBy: transaction.createdBy
            });
        }
        else {
            ...
            await prisma.user.updateMany({where: {id: {in: userIds}}, data: {points: {increment: amount}}});
            await prisma.event.update({
                where: {id},
            ...
            for (const data of dataArray) {
                ...
                results.push({id: t.id, recipient: t.utorid, awarded: amount,
                    type, relatedId: event.id, remark: event.description,
...
app.post("/promotions", jwtAuth, async (req, res) => {
    try {
        ...
        const { name, description, type, startTime, endTime, minSpending, rate, points, ...rest } = req.body;
        if (Object.keys(rest).length > 0) {
            return res.status(400).json({ error: `Unexpected fields: ${Object.keys(rest).join(", ")}` });
        }
        if (name === undefined || name === null) {
...
app.get("/promotions", jwtAuth, async (req, res) => {
    try {
        ...
        const promotions = await prisma.promotion.findMany({ where, skip, take: limit });
        const count = await prisma.promotion.count({ where });
        if (!clearance.includes(req.user.role)) {
            const results = promotions.map(p => {
                return {id: p.id, name: p.name, type: p.type, 
                    endTime: p.endTime, minSpending: p.minSpending, 
                    rate: p.rate, points: p.points
                };
            });
            return res.json({ count, results });
        }
            ...
            const results = promotions.map(p => {
                return {id: p.id, name: p.name, type: p.type, 
                    ...
                    minSpending: p.minSpending, rate: p.rate,
                    points: p.points
                };
            });
            return res.json({ count, results });
    ...
    try{
        ...
        if (!clearance.includes(req.user.role)) {
            return res.json({
                ...
                type: p.type, endTime: p.endTime, minSpending: p.minSpending, 
                rate: p.rate, points: p.points
            });
        }
        else {
...
app.patch("/promotions/:promotionId", jwtAuth, async (req, res) => {
    try {    
        ...
        if ((name || description || type || startTime || minSpending || rate || points) && now >= oldStart) {
            return res.status(400).json({ error: "too late" });
        } 
        if (start && end && start >= end) {
            return res.status(400).json({ error: "Invalid startTime, endTime" });
