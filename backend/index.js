...
app.post('/users', jwtAuth, async (req, res) => {
    try {
        ...
        if (!/^[a-zA-Z0-9]+$/.test(utorid)) {
            return res.status(400).json({
                error: 'utorid must only contain alphanumeric characters',
            });
        }
        ...
        if (name.length < 1 || name.length > 50) {
            return res
                .status(400)
                .json({ error: 'name must be 1-50 characters' });
        }
...
app.get('/users', jwtAuth, async (req, res) => {
    try {
        ...
        let { name, role, verified, activated, page, limit, ...rest } =
            req.query;
        ...
        if (verified !== undefined) {
            if (verified !== 'true' && verified !== 'false') {
                return res.status(400).json({
                    error: "Query parameter 'verified' must be a boolean.",
                });
            }
        ...
        if (activated !== undefined) {
            if (activated !== 'true' && activated !== 'false') {
                return res.status(400).json({
                    error: "Query parameter 'activated' must be a boolean.",
                });
            }
    ...
    try {
        ...
        if (email !== undefined && email !== null) {
            if (typeof email !== 'string') {
                return res
                    .status(400)
                    .json({ error: 'email must be a string' });
            }
        ...
        if (birthday) {
            ...
            if (
                ...
                return res.status(400).json({
                    error: 'Invalid birthday. Please check the month and day.',
                });
            }
        }
...
app.patch('/users/me/password', jwtAuth, async (req, res) => {
    try {
        ...
        if (!regex.test(newPassword)) {
            return res.status(400).json({
                error: 'New password must be 8-20 characters, at least one uppercase, one lowercase, one number, and one special character',
            });
        }
    ...
    try {
        ...
        if (verified !== undefined && verified !== null) {
            if (typeof verified !== 'boolean' || verified !== true) {
                return res
                    .status(400)
                    .json({ error: 'verified must be set to true.' });
            }
            data.verified = true;
        }
        ...
        if (role !== undefined && role !== null) {
            ...
            if (role === 'cashier' && suspicious === true) {
                return res
                    .status(400)
                    .json({ error: 'suspicious should be false.' });
            }
            if (
            ...
            if (
                !['regular', 'cashier', 'manager', 'superuser'].includes(role)
            ) {
                return res.status(400).json({
    ...
    try {
        ...
        if (type === 'purchase') {
            ...
            if (utorid === undefined || utorid === null) {
                return res
                    .status(400)
                    .json({ error: 'utorid must be provided' });
            }
            if (spent === undefined || spent === null) {
                return res
                    .status(400)
                    .json({ error: 'spent must be provided' });
            }
            const user = await prisma.user.findUnique({
                where: { utorid },
            });
            ...
            if (remark !== undefined && remark !== null) {
                if (typeof remark !== 'string') {
                    return res
                        .status(400)
                        .json({ error: 'remark must be a string' });
                }
                data.remark = remark;
            }
            const userData = {};
            ...
            if (promotionIds !== undefined && promotionIds !== null) {
                ...
                if (promotionIds.length > 0) {
                    if (user.used.some((id) => promotionIds.includes(id))) {
                        return res
                            .status(400)
                            .json({ error: 'promotion already used' });
                    }
                    const now = new Date();
                    ...
                    if (validPromotions.length !== promotionIds.length) {
                        return res.status(400).json({
                            error: 'One or more promotions are invalid, expired',
                        });
                    }
        ...
        if (relatedId === undefined || relatedId === null) {
            return res
                .status(400)
                .json({ error: 'relatedId must be provided' });
        }
        const user = await prisma.user.findUnique({
            where: { utorid },
        });
        ...
        if (!Number.isInteger(relatedId)) {
            return res
                .status(400)
                .json({ error: 'relatedID must be an integer' });
        }
    ...
    async (req, res) => {
        try {
            ...
            if (typeof suspicious !== 'boolean') {
                return res
                    .status(400)
                    .json({ error: 'suspicious must be boolean' });
            }
            const id = parseInt(req.params.transactionId);
            if (isNaN(id)) {
                return res.status(404).json({ error: 'Transaction not found' });
    ...
    async (req, res) => {
        try {
            ...
            if (processed !== true) {
                return res
                    .status(400)
                    .json({ error: 'processed must be true' });
            }
            const id = parseInt(req.params.transactionId);
            if (isNaN(id)) {
                return res.status(404).json({ error: 'Transaction not found' });
...
app.post('/events', jwtAuth, async (req, res) => {
    try {
        ...
        if (
            ...
            return res
                .status(400)
                .json({ error: 'Invalid startTime, endTime' });
        }
    ...
    try {
        ...
        if (endTime !== undefined && endTime !== null) {
            ...
            if (now >= oldEnd) {
                return res
                    .status(400)
                    .json({ error: 'Cannot change after event has ended' });
            }
        ...
        if (published !== undefined && published !== null) {
            ...
            if (published !== true) {
                return res
                    .status(400)
                    .json({ error: 'published must be true' });
            }
        ...
        if (
            ...
            return res
                .status(400)
                .json({ error: 'Cannot change after event has started' });
        }
    ...
    try {
        ...
        if (minSpending !== undefined && minSpending !== null) {
            if (
                ...
                return res.status(400).json({
                    error: 'minSpending must be a positive numeric value',
                });
            }
    ...
    try {
        ...
        if (endTime !== undefined && endTime !== null) {
            ...
            if (now >= oldEnd) {
                return res
                    .status(400)
                    .json({ error: 'Cannot change after promotion has ended' });
            }
            if (isNaN(end.getTime()) || end < now) {
                return res.status(400).json({ error: 'Invalid endTime' });
            }
        ...
        if (
            ...
            return res
                .status(400)
                .json({ error: 'Cannot change after promotion has started' });
        }
        if (start && end && start >= end) {
            return res
                .status(400)
    ...
    try {
        ...
        if (now >= start) {
            return res.status(403).json({
                error: 'Promotion cannot be deleted after it has started',
            });
        }
